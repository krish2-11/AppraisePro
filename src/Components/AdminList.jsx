    ...
    <div className="p-6 max-w-full">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">No Admin Found</h2>
          <p className="text-gray-500">There are no admin available at the moment.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {admin.map((obj , index) => (
            <Admin key={index} data={obj} />
          ))}
        </div>
      )}
    </div>
